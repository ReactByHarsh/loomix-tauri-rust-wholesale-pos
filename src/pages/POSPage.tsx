...
export const POSPage = () => {
    const { cart, addToCart, getTotal, clearCart, updateQuantity } = useCartStore();
    const { taxRate, taxEnabled, currency } = useSettingsStore();
    ...
    useBarcodeListener(async (code) => {
        ...
        if (visibleProduct) { if (visibleProduct.stock > 0) addToCart(visibleProduct); else alert("Out of stock!"); return; }
        // @ts-ignore
        if (window.api?.getProductBySku) {
            // @ts-ignore
            const product = await window.api.getProductBySku(code);
            if (product) { if (product.stock > 0) addToCart(product); else alert("Product out of stock!"); }
        }
    });
        ...
        try {
            ...
            if (result.success) {
                ...
                const receiptData = {
                    ...
                    subtotal, tax, taxRate, total: finalTotal, extraDiscount, paymentMethod, customerName: customer?.name, customerPhone: customer?.phone
                };
                setShowSuccess(true);
                setTimeout(() => {
        ...
        <div className="flex h-full bg-zinc-100 dark:bg-zinc-900 font-sans">
                ...
                <div className="p-3 shrink-0 bg-white dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <h1 className="text-base font-bold text-zinc-800 dark:text-zinc-100">POS Terminal</h1>
                                <p className="text-[10px] text-zinc-500">Scan or click to add</p>
                            </div>
                        </div>
                            ...
                            {filtered.map(product => (
                                <button key={product.id} onClick={() => product.stock > 0 && addToCart(product)} disabled={product.stock === 0}
                                    ...
                                    <div className="mt-auto flex items-center justify-between">
                                        <span className="font-bold text-zinc-800 dark:text-zinc-100 text-xs">{currencySymbol}{product.price}</span>
            ...
            <div className="flex-[0.4] bg-white dark:bg-zinc-800 flex flex-col h-full shadow-xl">
                ...
                <div className="p-3 shrink-0 border-b border-zinc-200 dark:border-zinc-700">
                    <div className="flex items-center justify-between mb-2">
                        <h2 className="text-sm font-bold text-zinc-800 dark:text-zinc-100">Order</h2>
                    ...
                    <div onClick={() => setShowCustomerModal(true)} className="w-full flex items-center justify-between p-2 rounded-lg border border-dashed border-zinc-200 dark:border-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-700/50 group cursor-pointer">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="text-left">
                                {customer ? (<><p className="text-xs font-medium text-zinc-800 dark:text-zinc-100">{customer.name}</p><p className="text-[10px] text-zinc-400">{customer.phone || 'No phone'}</p></>) : (<><p className="text-xs font-medium text-zinc-600 dark:text-zinc-300">Add Customer</p><p className="text-[10px] text-zinc-400">Optional</p></>)}
                            </div>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
                    {cart.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-300 dark:text-zinc-600 space-y-2">
                            ...
                            <p className="text-xs font-medium">Cart empty</p>
                        </div>
                    ) : (
                        cart.map((item) => {
                            return (
                                <div key={item.id} className="p-2 bg-zinc-50 dark:bg-zinc-900 rounded-lg">
                                    <div className="flex items-center gap-2">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    {item.cost_price ? (
                                                        <span className="text-[10px] text-zinc-300 dark:text-zinc-600 font-mono select-none">
                                                            645{Math.max(0, item.price - item.cost_price).toFixed(0)}
                                                        </span>
                                                    ) : null}
                                                    <span className="text-[10px] text-zinc-400">{currencySymbol}{item.price}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <span className="font-semibold text-zinc-800 dark:text-zinc-100 text-xs w-14 text-right">{currencySymbol}{(item.price * item.quantity).toFixed(0)}</span>
                                    </div>
                                </div>
                            );
                        })
                ...
                <div className="shrink-0 p-3 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 mb-2">
                        {[{ value: 'CASH', label: 'Cash', icon: Banknote }, { value: 'CARD', label: 'Card', icon: CreditCard }, { value: 'UPI', label: 'UPI', icon: QrCode }].map((method) => (
                    ...
                    <div className="mb-2 bg-white dark:bg-zinc-800 p-2 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-600">
                        <div className="flex items-center justify-between gap-2">
                            <span className="text-xs font-medium text-zinc-600 dark:text-zinc-300">Extra Discount</span>
                            <div className="relative">
                    ...
                    <div className="space-y-1 mb-3">
                        <div className="flex justify-between text-xs text-zinc-500"><span>Subtotal</span><span>{currencySymbol}{subtotal.toFixed(2)}</span></div>
                        <div className="flex justify-between text-xs text-zinc-500"><span>Tax ({taxRate}%)</span><span>{currencySymbol}{tax.toFixed(2)}</span></div>
                        {extraDiscount > 0 && (
                            <div className="flex justify-between text-xs text-red-500 font-medium"><span>Extra Discount</span><span>-{currencySymbol}{extraDiscount.toFixed(2)}</span></div>
                        )}

                        <div className="flex justify-between items-baseline pt-1.5 border-t border-zinc-200 dark:border-zinc-600">
                            <span className="font-bold text-zinc-800 dark:text-zinc-100 text-sm">Total</span>
                    ...
                    <div className="flex gap-2">
                        <button disabled={processing || cart.length === 0} onClick={() => handleCheckout(false)}
                            ...
                            {processing ? <span>...</span> : <><CreditCard size={14} /><span>Checkout</span></>}
                        </button>
                        <button disabled={processing || cart.length === 0} onClick={() => handleCheckout(true)}
                            ...
                            {processing ? <span>...</span> : <><Printer size={14} /><span>Print & Checkout</span></>}
                        </button>
                    </div>
                </div>
            </div>
            ...
            {showSuccess && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
                    <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-xl p-6 max-w-xs w-full text-center">
                        ...
                        <h3 className="text-lg font-bold text-zinc-800 dark:text-zinc-100">Success!</h3>
                        <p className="text-zinc-500 text-sm mt-1">Transaction recorded</p>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowCustomerModal(false)}>
                    <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-2xl p-4 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100">Add Customer</h3>
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-300 mb-1">Name *</label>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-300 mb-1">Date of Birth</label>
                                <input type="date" value={customerDob} onChange={(e) => setCustomerDob(e.target.value)}
                                    className="w-full px-3 py-2 border border-zinc-200 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-900 rounded-lg focus:outline-none focus:ring-1 focus:ring-zinc-400 text-zinc-800 dark:text-zinc-100 text-sm" />
                            </div>
                        </div>
                        <div className="flex gap-2 mt-4">
                            <button onClick={() => setShowCustomerModal(false)} className="flex-1 px-3 py-2 border border-zinc-200 dark:border-zinc-600 rounded-lg font-medium text-sm hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300">Cancel</button>
                            <button onClick={handleAddCustomer} disabled={!customerName.trim()} className="flex-1 px-3 py-2 bg-zinc-700 dark:bg-zinc-600 text-white rounded-lg font-medium text-sm hover:bg-zinc-600 dark:hover:bg-zinc-500 disabled:opacity-50">Add</button>
                        </div>
                    </div>
                </div>
            )}
