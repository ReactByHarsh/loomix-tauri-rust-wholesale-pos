...
use db::{Database, Product, TransactionData, TransactionDetails, TransactionHistoryResponse};
use license::LicenseManager;

// --- Database Commands ---

#[tauri::command]
fn get_transaction_history(db: State<Database>, page: i32, page_size: i32, search: Option<String>, payment_filter: Option<String>, date_filter: Option<String>) -> Result<TransactionHistoryResponse, String> {
...
#[tauri::command]
fn get_transaction_by_id(db: State<Database>, id: i32) -> Result<Option<TransactionDetails>, String> {
    db.get_transaction_by_id(id).map_err(|e| e.to_string())
}

#[tauri::command]
fn get_products(db: State<Database>, page: i32, page_size: i32, search: Option<String>, category: Option<String>) -> Result<Vec<Product>, String> {
    db.get_products(page, page_size, search.unwrap_or_default(), category.unwrap_or("all".to_string()))
        .map_err(|e| e.to_string())
}

#[tauri::command]
fn get_products_count(db: State<Database>, search: Option<String>, category: Option<String>) -> Result<i32, String> {
    db.get_products_count(search.unwrap_or_default(), category.unwrap_or("all".to_string()))
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
fn create_transaction(db: State<Database>, data: TransactionData) -> Result<serde_json::Value, String> {
...
#[tauri::command]
fn add_vendor_profile(db: State<Database>, profile: db::VendorProfile) -> Result<serde_json::Value, String> {
...
#[tauri::command]
fn get_vendors(db: State<Database>, page: i32, page_size: i32, search: Option<String>, date_filter: Option<String>, vendor_id: Option<i32>) -> Result<db::VendorListResponse, String> {
    db.get_vendors(
        page, 
        page_size, 
        search.unwrap_or_default(), 
...
#[tauri::command]
fn get_vendor_stats(db: State<Database>, vendor_id: Option<i32>) -> Result<db::VendorStats, String> {
...
#[tauri::command]
fn update_vendor(db: State<Database>, record: db::VendorRecord) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn activate_license(manager: State<'_, LicenseManager>, key: String) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn get_license_status(manager: State<'_, LicenseManager>) -> Result<license::LicenseData, String> {
...
#[tauri::command]
async fn retry_license_check(manager: State<'_, LicenseManager>) -> Result<serde_json::Value, String> {
